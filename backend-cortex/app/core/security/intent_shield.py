...
class IntentValidator:
    ...
    async def validate_intent(self, intent: str, action_summary: str, user_id: str = "Lien") -> Dict[str, Any]:
        ...
        logger.warning(f"🛡️ [SECURITY] Destructive Intent Detected: {intent} - {action_summary}")
        ...
        if context_verified:
            logger.info(f"🛡️ [SECURITY] Intent Validated by Context: {justification_found}")
            return {"valid": True, "reason": "Contextual justification found."}
        else:
            logger.error(f"🚨 [SECURITY] CONTEXT MISMATCH! No justification found for {intent}.")
            return {
                "valid": False,
