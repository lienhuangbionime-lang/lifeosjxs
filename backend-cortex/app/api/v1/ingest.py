...
async def auto_link_tasks_projects(content: str, db, http_request: Request) -> dict:
    """?™Â?ÊØîÂ??•Ë??ßÂÆπÔºåÊ?Ë®òÂ∑≤ÂÆåÊ??ÑÁèæ?â‰ªª?ôÔ?‰∏¶Êõ¥?∞Â?Ê°àÊ¥ªË∫çÁ??ã„Ä?""
    try:
        ...
        prompt = f"""
?ÜÊ?‰ΩøÁî®?ÖÁ??•Ë??ßÂÆπÔºåÂà§?∑‰??ë„ÄåÊòØ?¶Ê?Á¢∫Â??ê‰??çÊ??Æ‰??ÑÂì™‰∫õ‰ªª?ôÔ?‰ª•Â??åÊ??∞‰??çÂì™‰∫õÂ?Ê°à„Ä?

?êÁï∂?çÂ?Ëæ¶‰ªª?ôÊ??Æ„ÄëÔ?JSON ???ÔºåÂ???id ??titleÔº?
{json.dumps(active_tasks, ensure_ascii=False)}

?êÁï∂?çÊ¥ªË∫çÂ?Ê°àÊ??Æ„ÄëÔ?JSON ???ÔºåÂ???id ??nameÔº?
...
‰Ω†Á?‰ªªÂ?Ôº?
1. `completed_task_ids`: ÂæûÂ?Ëæ¶Ê??Æ‰∏≠ÔºåÊâæ?∫‰Ωø?®ËÄÖÂú®?•Ë?‰∏≠Ê?Á§∫‰??åÂ∑≤Á∂ìÂ??ê„ÄçÁ?‰ªªÂ? ID??
   ?†Ô? Ê•µÂ∫¶?çË?ÔºöÂ??àÊòØ?åÊ?Á¢∫Ë°®Á§∫Â∑≤Á∂ìÂ??ê„ÄÅÂ?ÂÆå„ÄÅÁ??ü„ÄçÁ?‰ªªÂ??ÇÂ??ú‰Ωø?®ËÄÖÂè™?ØÊ??∞„ÄåÊ≠£?®ÈÄ≤Ë??ÅÊé®?≤‰∏≠?ÅÊ?Â§©ÁπºÁ∫å„ÄçÔ??ñËÄÖÂ??úÊòØ Checkbox ‰ΩÜÊ??âÊ??æÔ?‰æãÂ? `- [ ]`ÔºâÔ?ÁµïÂ?‰∏çË??óÂÖ•ÔºÅÂ??úÊòØ?ìÂãæ?ÑÔ?Â¶?`- [x]`, `-[v]`ÔºâÊ?ÁÆóÂ??ê„Ä?
2. `mentioned_project_ids`: ÂæûÂ?Ê°àÊ??Æ‰∏≠ÔºåÊâæ?∫‰Ωø?®ËÄÖÂú®?•Ë?‰∏≠Ê??∞Á?Â∞àÊ? ID??

?ÖÂ??≥Âåπ?çÂà∞??ID Â≠ó‰∏≤????ÇËã•?°‰ªª‰ΩïÂåπ?çÔ??ûÂÇ≥Á©∫Èô£??[]??
"""
        ...
        if result_str:
            ...
            for p_id in mentioned_ids:
                ...
                p_name = next((p["name"] for p in active_projects if p["id"] == p_id), "?™Áü•Â∞àÊ?")
                if p_name != "?™Áü•Â∞àÊ?":
...
async def ingest_log(http_request: Request, request: IngestRequest, background_tasks: BackgroundTasks):
    ...
    logger.info(f"?ì• Received INGEST request. Source: {request.source} -> Target: {target_table}")
    ...
    # Use (?<!\d) and (?!\d) instead of \b to avoid Unicode word boundary issues (e.g. "2/1?•Ë?")
    import re as _re_date
    
    # 1. Match YYYY-MM-DD
    _date_explicit = _re_date.search(r'(?<!\d)(\d{4}-\d{2}-\d{2})(?!\d)', request.content)
    # 2. Match YYYYÂπ¥M?àD??
    _date_zh_full = _re_date.search(r'(\d{4})Âπ?\d{1,2})??\d{1,2})??, request.content)
    # 3. Match M?àD??(assume current year)
    ...
    if _date_explicit:
        ...
        logger.info(f"[OK] Date extracted from content (YYYYÂπ¥M?àD??: {ingest_date}")
    elif _date_zh_short:
        year = get_today_str_taipei()[:4]
        month = _date_zh_short.group(1).zfill(2)
        day = _date_zh_short.group(2).zfill(2)
        ingest_date = f"{year}-{month}-{day}"
        logger.info(f"[OK] Date extracted from content (M?àD??: {ingest_date}")
    elif _date_short:
    ...
    if multimodal_matches:
        logger.info(f"?îÆ Detected {len(multimodal_matches)} multimodal items in content. Processing interpretation...")
        ...
        for filename, data_url, mime_type in multimodal_matches:
            try:
                ...
                type_label = "?ì∏ Image" if "image" in mime_type else "?? Document"
    ...
    # Debug info
    logger.info(f"?ì• Received INGEST request. Mode: {request.mode}, skipAi: {request.skipAi}")
    
    try:
        # 1. ?ºÂè´ Gemma with v7.1 Prompt ??[SDK v1] google.genai (not google.generativeai)
        ...
        if not request.skipAi and model_config.get("configured") and req_gemma:
            # ÊßãÂª∫‰ΩøÁî®?ÖËº∏??(Ê≥®ÂÖ• Today ‰ΩúÁÇ∫?∫Ê??•Ê?)
        ...
        # --- ?∏Â??èËºØ‰øÆÊ≠£ÔºöÊó•?üË??ÜÊï∏Â∞çÈ? ---
        # 1. ?™Â?‰ΩøÁî® AI Ëæ®Ë??∫Á??•Ê?ÔºåÈÄôËÉΩËß?±∫Ë£úÂØ´?•Ë?ÔºàÂ? 2/1ÔºâÁ??èÈ?
        if not isinstance(ai_data, dict):
            ai_data = {}
        ...
        if not request.skipAi and ai_data:
            try:
                ...
                if "deep work" in markdown_body.lower() or "Ê∑±Â∫¶Â∑•‰?" in markdown_body:
                    proxy_facts.append({"type": "deep_work_session", "count": 1, "evidence": "keyword in markdown"})
                if "overtime" in markdown_body.lower() or "?†Áè≠" in markdown_body:
        ...
        # 4. ÂØ´ÂÖ• Supabase (Bypass if preview)
        if db and not request.preview:
            ...
            try:
                ...
                if tasks_list and target_table == "memories" and not request.preview:
                    ...
                    memory_id_db = None
                    if not tasks_list:
                        logger.info("?πÔ? No actionable tasks extracted from this diary entry.")
            ...
            try:
                ...
                logger.warning(f"?†Ô? Crystallization queue failed: {cryst_e}")
        ...
        if db and not request.skipAi:
            try:
                ...
                logger.warning(f"?†Ô? Auto-link failed, skipping: {link_e}")
        ...
        if db:
            try:
                ...
                if recon_res.data:
                    ...
                    logger.info("?ì° Sovereign Insights injected into response.")
            except Exception as recon_e:
                logger.warning(f"?†Ô? Failed to fetch recon insights: {recon_e}")
        ...
        import traceback
        traceback.print_exc()
        logger.error(f"?î• Ingest Critical Error: {str(e)}")
