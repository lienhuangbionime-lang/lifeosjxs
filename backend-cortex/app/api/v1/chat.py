...
def build_system_prompt(db, memory_context: str = "", document_context: str = "", growth_context: str = "", strategic_context: str = "", skills_context: str = "", hot_memory: str = "", cortex_context: str = "", radar_context: str = "", somatic_context: str = "") -> str:
    ...
    global_gemini_rules = ""
    try:
        with open(r"C:\Users\lien.huang\.gemini\GEMINI.md", "r", encoding="utf-8") as f:
            global_gemini_rules = f.read()
    except Exception as e:
        logger.warning(f"[WARN] Could not load Global GEMINI.md: {e}")
...
async def stream_chat(request: Request, payload: ChatRequest):
    ...
    try:
        ...
        # [v7.2] Switch to memory_service for Namespace Isolation & Dual-Query
        search_results = await memory_service.unified_search(
            question=payload.message,
            limit=5
        )
        memory_context = search_results.get("memories", "")
        # Documents are still handled separately by rag_service direct call if needed, 
        # or we update memory_service to include them. For now, unified_search is the lead.
        document_context = search_results.get("documents", "") 
        if not document_context:
             # Fallback to rag_service if memory_service didn't include docs
        ...
        system_instruction = build_system_prompt(
            db, 
            memory_context=_build_memory_context_block(memory_context), 
            document_context=_build_memory_context_block(document_context),
