...
async def get_contextual_prompts():
    ...
    try:
        ...
        if not memories and not latest_ref:
            return {"prompts": ["‰ªäÂ§©?éÂ?Â•ΩÂ?Ôº?, "?â‰?È∫ºÂÄºÂ?Ë®òÈ??ÑÂ?Ôº?, "ÂØ´‰?‰Ω†Áèæ?®Á??≥Ê??ßÔ?"]}
...
Output JSON: {"prompts": ["Q1", "Q2", "Q3"]}'''
        ...
        if not prompts:
            if latest_ref:
                prompts = ["?ÑË?ÂæóÊó©?çÁ?Ê∑±Â∫¶Ê¥ûÂ??éÔ?ÈªûÊ??ûÈ°ß...", "?úÊñº‰Ω†Á??êÈï∑Ê®°Â?ÔºåÊ?‰ªÄÈ∫ºÊñ∞?ºÁèæÔº?]
            elif memories:
                # Use a specific recent memory topic if possible
                latest_m = memories[0].get('content', '')[:20]
                prompts = [f"?úÊñº?é{latest_m}...?èÈ??â‰?È∫ºÊÉ≥Ë£úÂ??ÑÔ?", "‰ªäÂ§©?ÑÈÄ≤Â∫¶Â¶Ç‰?Ôº?, "?â‰?È∫ºÂÄºÂ?Á¥Ä?ÑÁ??éÔ?"]
            else:
                prompts = ["‰ªäÂ§©?éÂ?Â•ΩÂ?Ôº?, "?â‰?È∫ºÂÄºÂ?Ë®òÈ??ÑÂ?Ôº?, "Ê∫ñÂ?Â•ΩÈ?ÂßãÊñ∞?Ñ‰?Â§©‰??éÔ?"]
        
        return {"prompts": [p.replace("??, "").strip() for p in prompts[:3]]}
        
    except Exception as e:
        logger.error(f"Failed to generate contextual prompts (outer): {e}")
        return {"prompts": ["‰ªäÂ§©?éÂ?Â•ΩÂ?Ôº?, "?â‰?È∫ºÂÄºÂ?Ë®òÈ??ÑÂ?Ôº?]}
