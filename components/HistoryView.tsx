    ...
    return (
        <div className="h-full flex flex-col pb-20">
            <div className="flex justify-between items-center mb-6 px-2">
                <h2 className="text-2xl font-bold text-white flex items-center gap-2"><ListIcon className="text-indigo-400" /> Neural History</h2>
                <div className="relative">
                    <Filter className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 w-4 h-4"/>
                    <input type="text" placeholder="Filter..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9 pr-4 py-2 bg-slate-800 rounded-full text-xs text-white border border-slate-700 w-32 focus:w-48 transition-all outline-none"/>
                </div>
            </div>
                ...
                {currentLogs.map((log, i) => (
                    <div key={log.date + i} className="bg-[#1e293b] p-5 rounded-2xl border border-slate-700 hover:border-indigo-500/50 transition-colors group relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-3 relative z-10">
                            ...
                            <span className={`text-[10px] px-2 py-1 rounded border flex items-center gap-1 ${log.metrics?.mood >= 7 ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-slate-700 border-slate-600 text-slate-400'}`}>
                                <Activity size={10}/> {log.metrics?.mood ?? '-'}
                            </span>
                        </div>
                        ...
                        <div className="flex justify-between items-center pt-3 border-t border-slate-700/50 relative z-10">
                            <div className="flex gap-3 text-xs text-slate-500">
                                <span className="flex items-center gap-1" title="Energy"><Zap size={12}/> {log.metrics?.energy ?? '-'}</span>
                                <span className="flex items-center gap-1" title="Focus"><TrendingUp size={12}/> {log.metrics?.focus ?? '-'}</span>
                                <span className="flex items-center gap-1" title="Deep Work"><Clock size={12}/> {log.metrics?.deepWork ?? 0}h</span>
                            </div>
                        </div>
                    </div>
                ))}
